...
impl<T: Send> UnsafeAtomicRcBox<T> {
    ...
    /// 'killable' configures whether an unwrapping task can get interrupted
    /// by linked failure while waiting for other tasks to drop their
    /// references. Set to 'false' with caution.
    pub unsafe fn unwrap(self, killable: bool) -> T {
...
impl<T:Send> Exclusive<T> {
    ...
    pub fn unwrap(self) -> T {
        ...
        let inner = unsafe { x.unwrap(true) };
...
mod tests {
    ...
    fn arclike_try_unwrap_unwrap_race() {
        ...
        unsafe {
            ...
            do task::spawn {
                c.send(());
                assert!(x2.take().unwrap(true) == ~~"hello");
                c.send(());
            }
            p.recv();
